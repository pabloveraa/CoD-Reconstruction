...
p = uipanel(main_fig,'title','view 2','units','normalized', ...
    'position',[0.39 0.02 0.33 0.47],'fontsize',12);
x = [0.06 0.38 0.69];
y = [0.08 0.22 0.37 0.54 0.68 0.83];
w = 0.25;
h = 0.12;
...
nd = 10;
lambda_min = str2double(r.lambda_min.String);
lambda_max = str2double(r.lambda_max.String);
lambda = (lambda_min:(lambda_max - lambda_min)/(nd-1):lambda_max);
...
r.lambda_min.String = sprintf('%0.1e',pr.lambda(1));
r.lambda_max.String = sprintf('%0.1e',pr.lambda(2));
...
%==================================================
function plot_error(e_mean,e_std,lambda)
global r
figure('name','reconstruction_error','numbertitle','off');
plot(lambda,e_mean(:,1),'bo',lambda,e_mean(:,2),'ro','linewidth',2);
hold on;
grid on;
for i=1:length(lambda)
    x = [lambda(i), lambda(i)];
    y1 = [e_mean(i,1) - e_std(i,1), e_mean(i,1) + e_std(i,1)];
    y2 = [e_mean(i,2) - e_std(i,2), e_mean(i,2) + e_std(i,2)];
    plot(x,y1,'b-',x,y2,'r-','linewidth',2,'HandleVisibility','off');
end
set(gca,'fontsize',12);
if( r.units_m.Value==1 )
    units = '(m)';
...
xlabel('$\lambda$ (pixels$^{-2}$)','interpreter','latex','fontsize',16);
ylabel(strcat('$\epsilon ',units,'$'),'interpreter','latex','fontsize',16);
...
for i=1:n
    ...
    ptu1 = remove_distortion(ptd1,xd,yd,lambda);
    ptu2 = remove_distortion(ptd2,xd,yd,lambda);
...
if( abs(lambda)>1e-20 )
    r = sqrt((pts(:,1)-x0).^2 + (pts(:,2)-y0).^2);
    k = (1 - sqrt(1 - 4*lambda*r.^2))./(2*lambda*r.^2);
    xd = (pts(:,1) - x0).*k + x0;
    yd = (pts(:,2) - y0).*k + y0;
    ptd = [xd, yd];
    ind = (imag(k)~=0);
...
%=====================================================
function ptu = remove_distortion(ptd,xd,yd,lambda)
rd = sqrt((ptd(:,1)-xd).^2 + (ptd(:,2)-yd).^2);
xu = (ptd(:,1)-xd)./(1 + lambda*rd.^2) + xd;
yu = (ptd(:,2)-yd)./(1 + lambda*rd.^2) + yd;
ptu = [xu, yu];
